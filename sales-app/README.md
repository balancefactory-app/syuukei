...
- **スタッフ個別ログイン** — メールアドレス・パスワードでログイン、端末ごとに一度ログインすれば再ログイン不要
...
スタッフごとに個別のメールアドレス・パスワードでログインします。ログイン後はその端末にセッションが保存されるため、**同じ端末・同じブラウザであれば次回以降ログイン操作は不要**です（ブラウザのデータを消去した場合や「ログアウト」した場合は再ログインが必要）。

1. Firebase Console で **Authentication** > **Sign-in method** > **メール / パスワード** を有効化
2. Firebase Console の **Authentication** > **Users** タブから、スタッフ1人につき1アカウントを作成
   - 「ユーザーを追加」→ メールアドレス（例: `takei@example.com`）とパスワードを設定
   - スタッフの人数分繰り返す
3. 作成したメールアドレス・パスワードを各スタッフに個別に伝える

退職などでアカウントを無効化したい場合は、**Authentication** > **Users** から該当ユーザーを削除すれば、その場でログインできなくなります。

パスワードを忘れた場合は、Firebase Console のユーザー一覧からパスワードをリセットできます。
...
ログインは端末ごとに一度だけ必要です（スタッフ個別のメールアドレス・パスワードを入力）。一度ログインすれば、ホーム画面のアイコンから次回以降ログイン操作なしで開けます。
